...
export default function App() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-animated-gradient">
      ...
      {/* ── Main chat panel ───────────────────────────────────── */}
      {/* Grid rows: [header+banner]=auto, [chat feed]=1fr (hard-capped, scrolls internally), [input]=auto.
          Grid rows are explicit sizes rather than relying on flex-shrink math, so ChatFeed can
          never grow past its row and push ChatInput off-screen, no matter how tall a message is. */}
      <main className="flex-1 min-w-0 h-full grid grid-rows-[auto_1fr_auto] overflow-hidden">
        {/* Header + warning banner (auto-sized row) */}
        <div>
          ...
          {noDocuments && (
            <div className="mx-4 mt-4 px-4 py-3 rounded-xl bg-amber-500/10 border border-amber-500/20
              text-amber-300 text-sm flex items-center gap-2 animate-fade-in">
        ...
        {/* Chat feed (scrollable, hard-capped to its grid row) */}
        <ChatFeed
          messages={messages}
          streaming={streaming}
        ...
        {/* Chat input (auto-sized row, always visible) */}
        <ChatInput
          onSend={sendMessage}
