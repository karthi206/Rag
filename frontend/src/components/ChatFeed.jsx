...
export default function ChatFeed({ messages, streaming, streamText }) {
  ...
  const stickToBottomRef = useRef(true) // whether we should auto-follow new content

  // Track whether the user is near the bottom; if they've scrolled up
  // on purpose, stop yanking them back down.
  const handleScroll = () => {
    const el = containerRef.current
    if (!el) return
    const distanceFromBottom = el.scrollHeight - el.scrollTop - el.clientHeight
    stickToBottomRef.current = distanceFromBottom < 120 // px threshold
  }

  // Auto-scroll to bottom on new messages / stream updates,
  // but only if the user hasn't scrolled away, and without
  // fighting the scroll with repeated 'smooth' animations.
  useEffect(() => {
  ...
  // When a brand-new message is added (not just a token update),
  // snap the stickiness back on and do one nice smooth scroll.
  useEffect(() => {
  ...
  return (
    <div
      ...
      className="min-h-0 overflow-y-auto px-4 py-6 space-y-6">
      {isEmpty && (
